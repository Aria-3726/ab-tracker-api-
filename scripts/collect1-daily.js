...
async function main() {
  ...
  for (let i = 0; i < CREATORS.length; i++) {
    ...
    // Append with view data
    const entry = { d: today, yt: ytSubs, tt: ttFollowers, ytTotalViews: ytTotalViews };
    if (ytDailyViews !== null) entry.ytv = ytDailyViews;
    ...
    console.log(`  ${c.name}: YT=${ytSubs}, TT=${ttFollowers !== null ? ttFollowers : 'N/A'}, YTViews=${ytDailyViews !== null ? ytDailyViews : '(first day)'}`);
  }
